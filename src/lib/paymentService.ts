...
const getPaymentApiBaseUrl = (): string => {
  ...
  if (typeof window !== "undefined" && window.location) {
    ...
    console.warn("VITE_PAYMENT_API_BASE_URL is not configured. Falling back to window.location.origin.", fallback);
    return fallback;
  }
  return "";
};
...
export const verifyRazorpayPaymentAndCreatePrebooking = async (input: {
  ...
  const currentUser = auth.currentUser;
  if (!currentUser) throw new Error("You must be logged in to verify payment.");
...
export const deleteNfcProfileDraft = async (profileId: string): Promise<void> => {
  ...
  if (!res.ok) { const text = await res.text(); throw new Error(text || "Failed to delete NFC draft."); }
};
...
export const downloadReceipt = async (prebooking: PrebookingData, invoiceEmail: string): Promise<void> => {
  const invoiceData = buildInvoiceDataFromPrebooking(prebooking, {
    ...
    qrCodeUrl: `https://plzpingme.com/orders/${encodeURIComponent(prebooking.payment?.orderId || "")}`,
    locale: "en-IN",
  });
  void invoiceEmail; // used by caller for display only
  const blob = await buildInvoicePdfBlob(invoiceData);
  const url = URL.createObjectURL(blob);
  const link = document.createElement("a");
  link.href = url;
  link.download = `pingme-invoice-${invoiceData.orderId.replace(/[^a-zA-Z0-9_-]/g, "")}.pdf`;
  link.style.display = "none";
  document.body.appendChild(link);
  if (typeof MouseEvent === "function") link.dispatchEvent(new MouseEvent("click"));
  else link.click();
...
export const openRazorpayCheckout = async (input: {
  ...
  if (!keyId) throw new Error("VITE_RAZORPAY_KEY_ID is missing.");
  ...
  const rzp = new window.Razorpay({
    ...
    handler: (response) => { void input.onSuccess(response); },
    prefill: { name: input.fullName, email: input.email, contact: input.phone },
    notes: { source: "pingme-prebook" },
    theme: { color: "#f4c300" },
