  ...
  return (
    ...
    >
      <style>{`
        ...
        /* Pulse only the icon to avoid moving the badge */
      ...
      {count > 0 && (
        <span className="pm-cart-badge" aria-hidden>{count > 99 ? "99+" : count}</span>
      )}
    </button>
  );
};

function numberOrZero(v: any) { const n = Number(v); return Number.isFinite(n) ? Math.max(0, Math.floor(n)) : 0; }
